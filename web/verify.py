...
fontType = config_data.get('verify_font')
...
def create_validate_code(size=(120, 30),
    ...
    if draw_arclines:
        create_arcline(draw, n_line, width, height)
...
def create_arcline(draw, n_line, width, height):
    '''绘制干扰曲线'''
    line_num = random.randint(n_line[0], n_line[1])  # 干扰线条数
    for i in range(line_num):
        # 位置
        position = (random.randint(-width, 0), random.randint(-width, 0), random.randint(0, width), random.randint(0, width))
        draw.arc(position, random.randint(0, 360), random.randint(0, 360), fill=create_bg())
...
def create_strs(draw, chars, length, font_type, font_size, width, height, fg_color):
    ...
    font_width, font_height = font.getsize(c_chars[0])
    x = width / length - 6

    for i in range(length):
        draw.text((x * i + 14, (height - font_height) / 3) , c_chars[i], font=font, fill=fg_color)
